...
public class AnimGraphViewModel
{
    ...
    /// <summary>
    /// 既存の汎用 Asset Graph 呼び出しとの互換入口です。AnimBlueprint 以外は
    /// 呼び出し側のフォールバックノード生成へ委ねます。
    /// </summary>
    public static AnimGraphViewModel ExtractFromObject(UObject asset)
        => asset is UAnimBlueprintGeneratedClass animBlueprintClass
            ? ExtractFromClass(animBlueprintClass)
            : new AnimGraphViewModel { PackageName = asset?.Owner?.Name ?? asset?.Name ?? string.Empty };
    ...
    private static List<(AnimGraphNode rootNode, string layerName)> ResolveGraphLayerRoots(
        ...
        Dictionary<string, AnimGraphNode> nodeByName)
    {
        if (animBlueprintClass is UAnimBlueprintGeneratedClass generatedClass)
            return ResolveGraphLayerRoots(generatedClass, nodeByName);
    ...
    private static bool IsAnimNodeStruct(FStructProperty property, string fieldName)
    {
        var structName = property.Struct.ResolvedObject?.Name.Text ?? string.Empty;
        return IsAnimNodeStruct(structName) || IsAnimNodeStruct(fieldName);
    }

    private static bool IsAnimNodeStruct(string name)
        => name.StartsWith("FAnimNode_", StringComparison.OrdinalIgnoreCase) ||
    ...
    private static bool IsSaveCachedPoseNode(AnimGraphNode node)
        => node.ExportType.Contains("SaveCachedPose", StringComparison.OrdinalIgnoreCase) ||
    ...
    private static void ExtractNodeProperties(UObject cdo, string propName, AnimGraphNode node)
    {
        ...
        foreach (var prop in structValue.Properties)
        {
            ...
            if (name.Equals("NodeComment", StringComparison.OrdinalIgnoreCase))
                node.NodeComment = value;
            else if (value.Length <= MaxPropertyValueDisplayLength)
    ...
    private static bool IsLinkedNodeProperty(string name)
        => name.Equals("BasePose", StringComparison.OrdinalIgnoreCase) ||
    ...
    private static void ResolveConnections(UObject cdo,
        List<(string name, string structType, int childPropertyIndex)> animNodeProps,
        Dictionary<string, AnimGraphNode> nodeByName, AnimGraphViewModel vm)
    {
        ...
        {
            if (!cdo.TryGetValue(out FStructFallback structValue, propName) ||
                !nodeByName.TryGetValue(propName, out var sourceNode))
                continue;

            foreach (var prop in structValue.Properties)
            {
                if (prop.Tag != null)
                    TryResolvePoseLink(prop.Tag, prop.Name.Text, sourceNode, animNodeProps, nodeByName, vm);
            }
        }
    }
        ...
        Dictionary<string, AnimGraphNode> nodeByName, AnimGraphViewModel vm)
    {
        ...
        if (tag.GetValue(typeof(FStructFallback)) is not FStructFallback linkStruct ||
            !linkStruct.TryGetValue(out int linkId, "LinkID") ||
            linkId < 0 || linkId >= animNodeProps.Count)
            return;

        var targetName = animNodeProps[linkId].name;
        if (targetName == sourceNode.Name || !nodeByName.TryGetValue(targetName, out var targetNode))
            return;

        sourceNode.Pins.Add(new AnimGraphPin
        {
            PinName = pinName,
            IsOutput = false,
            PinType = "pose",
