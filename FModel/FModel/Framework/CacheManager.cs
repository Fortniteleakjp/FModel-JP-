...
public static class CacheManager
{
    ...
    {
        if (!newPath.StartsWith(MappingsDirectory, StringComparison.OrdinalIgnoreCase) ||
            UserSettings.Default.PerDirectory is null)
            return;

        foreach (var directorySettings in UserSettings.Default.PerDirectory.Values)
        {
            if (directorySettings.Endpoints is null)
                continue;
