...
namespace CUE4Parse.UE4.Exceptions
{
    ...
    public class ParserException : Exception
    {
        ...
        public ParserException(FArchive reader, string? message = null, Exception? innerException = null)
            : base($"{message}\n{reader.GetType().Name} Info: {reader.Name} | Pos:{reader.Position} Length:{reader.Length} ({(reader.Length == 0 ? "0%" : $"{Math.Round((decimal)reader.Position / reader.Length * 100, 1)}%")} done)", innerException)
        { }
    }
}
