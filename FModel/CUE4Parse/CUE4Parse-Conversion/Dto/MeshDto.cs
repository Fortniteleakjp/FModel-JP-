...
// PR #358 back-port — 段階導入(L3):
// 本家 head の MeshDto.cs は StaticMeshDto / SkeletonDto / SkeletalMeshDto / LandscapeMeshDto を1ファイルに含むが、
// ここでは依存が整っている「静的メッシュ経路」のみを移植する。Skeletal / Landscape / Nanite は後続レイヤーで追加する。
...
public class StaticMeshDto : MeshDto<MeshVertex>
{
    ...
    // Spline メッシュ: 参照する静的メッシュをロードしてスプライン変形付きで構築。
    public StaticMeshDto(USplineMeshComponent spline) : this(spline.GetStaticMesh().Load<UStaticMesh>() ?? throw new ArgumentNullException(nameof(spline), "Spline mesh has no static mesh"), ENaniteMeshFormat.OnlyNormalLODs, spline)
    {
    }

    // NOTE: ENaniteMeshFormat は JP 既存の enum(CUE4Parse.UE4.Assets.Exports.Nanite)を使用。
    //   意味対応(head→JP): NaniteOnly=OnlyNaniteLOD / NoNanite=OnlyNormalLODs / NaniteFirst=AllLayersNaniteFirst / NaniteLast=AllLayersNaniteLast
    public StaticMeshDto(UStaticMesh mesh, ENaniteMeshFormat naniteFormat = ENaniteMeshFormat.OnlyNormalLODs, USplineMeshComponent? spline = null) : base(mesh)
    {
        ...
        if (naniteFormat != ENaniteMeshFormat.OnlyNaniteLOD) // just so we don't waste time
        {
        ...
        else if (LODs.Count == 0) // in case OnlyNaniteLOD was requested but there was no nanite to parse
        {
            ParseMeshRenderData(mesh.RenderData, spline);
        }
    }
