...
public class SceneComponentDto : ComponentDto
{
    ...
    {
        // PR #358 back-port (JP適合): JP の USceneComponent は AttachParent プロパティを持たないため GetOrDefault で読む。
        _attachParent = component.GetOrDefault<FPackageIndex?>("AttachParent");
...
public abstract class MeshComponentDto(FPackageIndex meshPtr, UMeshComponent component, ActorDto owner) : PrimitiveComponentDto(component, owner)
{
    ...
    // PR #358 back-port (JP適合): JP の UMeshComponent は OverrideMaterials プロパティを持たないため GetOrDefault で読む。
    public readonly FPackageIndex?[] OverrideMaterials = component.GetOrDefault<FPackageIndex?[]>("OverrideMaterials", []);
}
...
public class LandscapeSplinesComponentDto : PrimitiveComponentDto
{
    ...
    {
        // PR #358 back-port (JP適合): JP には ULandscapeSplineSegment 型と ULandscapeSplinesComponent.Segments が無いため、
        // セグメント/ローカルメッシュを GetOrDefault による文字列ルックアップで読み出す。
        foreach (var ptr in component.GetOrDefault<FPackageIndex?[]>("Segments", []))
        {
            if (ptr?.TryLoad<UObject>(out var segment) == true && segment != null)
            {
                foreach (var meshPtr in segment.GetOrDefault<FPackageIndex?[]>("LocalMeshComponents", []))
                {
