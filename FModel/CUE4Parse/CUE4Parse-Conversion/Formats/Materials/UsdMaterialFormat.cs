...
public sealed class UsdMaterialFormat : IMaterialExportFormat
{
    ...
    private bool TryResolveTexture(CMaterialParams2 parameters, string[] names, string packageDirectory, out string path)
    {
        ...
        {
            // NOTE: レガシー CUE4Parse_Conversion.ExporterBase との衝突を避けるため Exporters. で明示修飾。
            path = Exporters.ExporterBase.Resolve(texture, packageDirectory, "png"); // TODO: not only PNG?
            return true;
        }
        path = string.Empty;
        return false;
